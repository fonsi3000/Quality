    ...
    <div class="bg-white rounded-xl shadow p-4 sm:p-7 dark:bg-neutral-800">
        ...
        <form action="{{ route('users.update', $user) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid sm:grid-cols-12 gap-2 sm:gap-6">
                ...
                <!-- Segundo proceso -->
                <div class="sm:col-span-3">
                    <label for="second_process_id" class="inline-block text-sm text-gray-800 mt-2.5 dark:text-neutral-200">Segundo proceso (opcional)</label>
                </div>
                <div class="sm:col-span-9">
                    <select id="second_process_id" name="second_process_id"
                            class="py-2 px-3 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-400 dark:focus:ring-neutral-600 @error('second_process_id') border-red-500 @enderror">
                        <option value="">Seleccione un segundo proceso</option>
                        @foreach($processes as $process)
                            <option value="{{ $process->id }}" {{ old('second_process_id', $user->second_process_id) == $process->id ? 'selected' : '' }}>{{ $process->name }}</option>
                        @endforeach
                    </select>
